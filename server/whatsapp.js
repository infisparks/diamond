...
const FIREBASE_DB_URL = (process.env.FIREBASE_DATABASE_URL || process.env.NEXT_PUBLIC_FIREBASE_DATABASE_URL || "https://diamond-78d8d-default-rtdb.firebaseio.com").replace(/\/$/, "");
...
async function sendMetaCloudApiFounderNotification({ fullName, email, phone, bookingTime }) {
  try {
    ...
    const rawFounderNumbers = process.env.META_WA_FOUNDER_NUMBERS || "917709439688";
...
router.post("/notify-admin-ticket", async (req, res) => {
  try {
    ...
    if (adminPhones.size === 0) {
      adminPhones.add("917709439688"); // Default Master Admin
    }
